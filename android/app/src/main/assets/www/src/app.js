...
function apiEndpoint(path, params) {
  ...
  const functionName = path.includes('estaciones') || path.includes('stations-nearby') ? 'stations-nearby' : path.replace(/^\//, '');
  return `${base}/${functionName}?${params}`;
}
...
async function testServerConnection() {
  ...
  try {
    ...
    const payload = await apiFetch('/stations-nearby', params);
...
function bind() {
  ...
  el.fullTankButton?.addEventListener('click', () => runFullTankSearch({ openRoute: true }));
...
function init() {
  ...
  try {
    if (window.AndroidBridge?.consumeFullTankLaunch?.()) {
      setTimeout(() => runFullTankSearch({ openRoute: true }), 900);
    }
